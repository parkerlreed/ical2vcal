...
Improvements copyright (c) 2007 Charl P. Botha http://cpbotha.net/
...
# cpbotha changed + in second to last line to *, else it broke badly with
# zero-length property values... (as sometimes generated by google cal)
...
class Event:
    ...
    def as_vcal(self):
        ...
        for attribute in self.event_attributes():
            ...
            if attribute == "klass":
                attribute = "class" # typo fixed by cpbotha

            # really ugly rrule transformation by cpbotha
            # let me know at http://cpbotha.net/ if this does NOT work for you.
            # see this wunnerful guide:
            # http://www.shuchow.com/vCalAddendum.html
            freq = ''
            freqtype = ''
            interval = '1'
            fmod = ''
            dur = '20311231' # forever
            if attribute == "rrule":
                # do stuff.
                for rr in val:
                    for i in rr.split(';'):
                        ...
                        if k == 'freq':
                            ...
                            freqtype = 'p' # position modifier, i.e. which day of the week
                            # byday consists of %d%s,%d%s,%d%s where %d is optional
                            for b in v.split(','):
                                mo = re.match('([\+\-]*)([0-9]*)([a-z]*)',b)
                                if mo:
                                    sgn, ith, day = mo.groups()
                                    if not sgn:
                                        sgn = '+'
                ...
                if freqtype == '':
                    ...
                    if freq == 'y':
                        freqtype = 'm' # i'th month of the year
                        fmod = parsetime(self.dtstart)[1] # get month

                    # and default for monthly                        
                    elif freq == 'm':
                        freqtype = 'd' # by-day modifier, i.e. i'th day of the month
                        fmod = parsetime(self.dtstart)[2] # day of month
                        
                val = ('%s%s%s %s %s' % (freq, freqtype, interval, fmod, dur)).strip().upper()
                                
                                
                          
                
            # cpbotha added str(), as duration_seconds is not a string!
            val = str(val)

            # If a long line, fold long at whitespace (vcal requirement)
            # (cpbotha: with plural_props, this of course gets another meaning,
            #  but I've turned everything into strings in anycase...)
            if len(val) > 75:
                rest = val
                l1 = []
                while len(rest) > 75:
            ...
            # we only output the field if it has a value. (cpbotha)
            if len(val) > 0:
